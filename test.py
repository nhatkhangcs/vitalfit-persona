...
with open(os.path.join(os.path.dirname(__file__), 'config', 'prompts', 'valid.txt'), 'r', encoding='utf-8') as f:
    VALIDATOR_PROMPT = f.read()
with open(os.path.join(os.path.dirname(__file__), 'config', 'prompts', 'orchestrator.txt'), 'r', encoding='utf-8') as f:
    ORCHESTRATOR_PROMPT = f.read()
with open(os.path.join(os.path.dirname(__file__), 'config', 'prompts', 'nutrition.txt'), 'r', encoding='utf-8') as f:
    NUTRITION_PROMPT = f.read()
with open(os.path.join(os.path.dirname(__file__), 'config', 'prompts', 'schedule.txt'), 'r', encoding='utf-8') as f:
...
async def on_quit_action(action):
    await cl.Message(content="👋 **Bạn đã kết thúc phiên trò chuyện. Cảm ơn bạn đã sử dụng Hệ thống hỗ trợ đau lưng!**").send()
    global validated
    validated = False
    # reset memory
    await action.remove()
    await cl.Message(content=f"Server 6379: {memory_handler_user.clear_memory()}").send()
    await cl.Message(content=f"Server 6380: {memory_handler_other.clear_memory()}").send()
...
async def set_starters():
    return [
        cl.Starter(
            ...
            message="Bắt đầu trò chuyện với Hệ thống hỗ trợ đau lưng",
...
async def main(message: cl.Message):
    # receive pdf as second multimodality
    ...
    print("Have PDFs: ", pdfs == [])

    images = [file for file in message.elements if ("png" or "jpg") in file.mime]

    text_in_pdf += "=== PDF Informaion End ==="
    global validated

    message_with_context_user = memory_handler_user.get_history_message(text_in_pdf + "\n" + message.content)

    actions = [
        cl.Action(name="quit", label="❌ Kết thúc", tooltip="Kết thúc phiên trò chuyện",payload={})
    ]
    ...
    try:
        if not validated:
            ...
            response = await validator_agent.run([message_with_context_user + f"\nCURRENT QUESTION:\n {text_in_pdf} \n {message.content}"])
            ...
            if 'ok' in str(response.output).strip().lower():
                ...
                orchestrator_response = await orchestrator_agent.run(message_with_context_user + f"\nCURRENT QUESTION:\n + {text_in_pdf} '\n' {message.content}")
                ...
                if "nutrition" in str(orchestrator_response).strip().lower():
                    ...
                    nutrition_response = await nutrition_agent.run(message_with_context_user + '\n' + str(orchestrator_response).strip().lower() +  f"\nCURRENT QUESTION:\n {text_in_pdf} '\n' {message.content}")
                    memory_handler_other.store_bot_response(nutrition_response.output)

                    await cl.Message(
                        content=f"{nutrition_response.output}",
                        actions=actions
                    ).send()
                    ...
                    schedule_response = await schedule_agent.run(message_with_context_user +  '\n' + str(orchestrator_response).strip().lower() +  f"\nCURRENT QUESTION:\n {text_in_pdf} '\n' {message.content}")
                    memory_handler_other.store_bot_response(schedule_response.output)

                    await cl.Message(
                        content=f"{schedule_response.output}",
                        actions=actions
                    ).send()

                else:
                    await cl.Message(content=str(orchestrator_response.output), actions=actions).send()
            ...
            message_with_context_other = memory_handler_other.get_history_message(message.content)
            orchestrator_response = await orchestrator_agent.run(message_with_context_user + "\n" + message_with_context_other + f"\nCURRENT QUESTION:\n {text_in_pdf} '\n' {message.content}")
            memory_handler_other.store_bot_response(orchestrator_response.output)
        ...
        await cl.Message(content=f"❌ **Lỗi:** {str(e)}\n\nVui lòng thử lại.", actions=actions).send()