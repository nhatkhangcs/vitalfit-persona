...
def send_email_tool(
    ...
    try:
        ...
        print(f"Using sender password: {'*' * len(sender_password) if sender_password else 'Not provided'}")
        ...
        with smtplib.SMTP(smtp_server, smtp_port) as server:
            ...
            server.sendmail(sender_email, input_data.to_emails, message.as_string())

        print(f"Email sent successfully to {', '.join(input_data.to_emails)}")

        return EmailToolOutput(
            success=True, message=f"Email sent to {', '.join(input_data.to_emails)}"
        )
...
def create_send_email_tool():
    ...
    @cl.step(type="tools")
    async def configured_send_email_tool(input_data: EmailToolInput) -> EmailToolOutput:
